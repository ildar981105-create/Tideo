...
(function() {
    ...
    window.TideoTaskPanel = {
        ...
        _syncSubmitButton: function() {
            ...
            if (running) {
                btn.classList.add('pbi-submit--stop');
                btn.innerHTML = '<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><rect x="1" y="1" width="10" height="10" rx="2" fill="currentColor"/></svg>';
                btn.title = '停止当前任务';
            } else {
                btn.classList.remove('pbi-submit--stop');
                btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M3 8H13M13 8L9 4M13 8L9 12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
                btn.title = '发送';
            }
        },
        ...
        _bindEvents: function(skill) {
            ...
            submitBtn.addEventListener('click', function() {
                ...
                if (window.TideoTaskPanel && window.TideoTaskPanel.isRunning()) {
                    document.dispatchEvent(new CustomEvent('tideo:stop', {
                        detail: { skill: skill }
                    }));
                    return;
                }
